...
export const BrandInkOpen: React.FC = () => {
  ...
  const rowStyle = {
    fontFamily: FONT_DISPLAY, fontWeight: 600, color: INK, letterSpacing: '-0.01em',
    lineHeight: 1.12, whiteSpace: 'pre' as const, display: 'inline-flex' as const,
    justifyContent: 'center' as const, alignItems: 'flex-end' as const,
    fontSize: isVertical ? 86 : 118,
  };

  // Two explicit lines ("Malerei" / "Scheither") at a smaller size in the
  // 9:16 cutdown: at full size this single line ran well past the vertical
  // crop's ~608px-wide safe window, and letting it flex-wrap instead broke
  // mid-word ("Schei"/"ther") rather than at the space. Splitting explicitly
  // keeps the per-glyph reveal timing (still indexed off the full string)
  // but always breaks cleanly at the word boundary.
  const spaceIdx = WORDMARK.indexOf(' ');
  const wordmarkEl = isVertical ? (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <div style={rowStyle}>{WORDMARK.slice(0, spaceIdx).split('').map((ch, i) => renderChar(ch, i))}</div>
      <div style={rowStyle}>{WORDMARK.slice(spaceIdx + 1).split('').map((ch, i) => renderChar(ch, spaceIdx + 1 + i))}</div>
    </div>
  ) : (
    <div style={rowStyle}>{WORDMARK.split('').map(renderChar)}</div>
  );
