...
export const TrustList: React.FC = () => {
  ...
  return (
    // narrow (860px, centred, stacked dot-over-text) rather than a wide
    // left-aligned row so the block survives the 9:16 centre-crop too
    <AbsoluteFill style={{ backgroundColor: PAPER, justifyContent: 'center', alignItems: 'center' }}>
      <div style={{ width: 860, display: 'flex', flexDirection: 'column', gap: 22, transform: `translateY(${drift}px)` }}>
        {ITEMS.map((text, i) => {
          const start = 10 + i * 16;
          ...
          const scale = 0.86 + Math.min(1, Math.max(0, p)) * 0.14;
          ...
          return (
            ...
            >
              ...
              <span style={{ fontFamily: FONT_SANS, fontWeight: 500, fontSize: 30, lineHeight: 1.32, color: INK }}>{text}</span>
            </div>
          );
        })}
      </div>
